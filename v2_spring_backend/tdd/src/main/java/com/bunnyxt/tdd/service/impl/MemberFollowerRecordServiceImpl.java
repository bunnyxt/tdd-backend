...
public class MemberFollowerRecordServiceImpl implements MemberFollowerRecordService {
    ...
    @Override
    public List<MemberFollowerRecord> queryMemberFollowerRecords(Integer mid, Integer start_ts, Integer end_ts, Integer pn, Integer ps) {
        // pn, ps -> offset, ps
        ps = PageNumModfier.modifyPs(ps, 25000);
        pn = PageNumModfier.modifyPn(pn);
        Integer offset = PageNumModfier.calcOffset(ps, pn);

        return memberFollowerRecordDao.queryMemberFollowerRecords(mid, start_ts, end_ts, offset, ps);
    }
